...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️', url='https://t.me/bithjbunny')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('👑Source Bot', url='tg://openmessage?user_id=5512543458'),
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/bithjbunny'),
            ]
            ]
